...
optimizer_params = {
    ...
    'scheduler': {
        'milestones': [50,60,70,80,90,100,110,120,130,140,150,160,170,180,190],
        'gamma': 0.9
    }
}
...
trainer_params = {
    ...
    'epochs': 200,
    'train_batch_size': 4,   # maximum number of problems in one batch
    ...
    'model_load': {
        'enable': True,  # enable loading pre-trained p1 model
        'path': './POMO/result/p1-checkpoints',  # directory path of pre-trained model and log files saved.
        'epoch': 55,  # epoch version of p1 model to laod.
...
logger_params = {
    'log_file': {
        'desc': 'finetune(phase2)_with_setX',
        'filename': 'run_log'
    }
}
if env_params['use_early_stopping']:
    env_params['early_stopping_threshold']=(trainer_params['logging']['model_save_interval']/trainer_params['epochs'])*0.5
##########################################################################################
# main
import time
