...
class TSPEnv:
    ...
    def sampling_subpaths(self, problems, solution, length_fix=False, mode='test', repair=False):
        ...
        new_sulution_ascending, rank = torch.sort(new_sulution, dim=-1, descending=False)  # in an ascending order
        _, new_sulution_rank = torch.sort(rank, dim=-1, descending=False)  # in an ascending order
    ...
    def make_tsplib_data(self, filename,episode):
        ...
        instance_data = np.array(instance_data)  # Each row of data represents an instance, and the size of each instance is different.s
        cost = np.array(cost)
