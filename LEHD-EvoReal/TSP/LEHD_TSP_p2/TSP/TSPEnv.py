...
class TSPEnv:
    ...
    def load_problems(self, episode, max_batch_size):
        ...
        # 1. Get the raw instance (single problem)
        problem_nodes = torch.tensor(self.raw_data_nodes[episode])
        problem_tour = torch.tensor(self.raw_data_tours[episode])

        # 2. Add batch dimension and expand to batch_size copies
        self.problems = problem_nodes.unsqueeze(0).expand(max_batch_size, -1, -1).contiguous()  # [B, V, 2]
        self.solution = problem_tour.unsqueeze(0).expand(max_batch_size, -1).contiguous()       # [B, V]

        # 3. Independently sample subpaths for each batch sample
        if self.sub_path:
            self.problems, self.solution = self.sampling_subpaths(self.problems, self.solution, mode='train')

        # 4.  Randomly decide whether to flip; all batches are either flipped or not (for per-batch independent flipping, use a loop or vectorize)
        if_inverse = True
        ...
        # 5. Update the current problem size
        self.problem_size = problem_nodes.shape[0]
        self.problem_batch_size = self.batch_size
    ...
    def sampling_subpaths(self, problems, solution, length_fix=False, mode='test', repair=False):
        ...
        min_subpath_len = 4
        max_start = problems_size - min_subpath_len
    
        first_node_index = torch.randint(low=0, high=max_start + 1, size=[1]).item()
    
        if length_fix:
            length_of_subpath = problems_size
        else:
            max_len = problems_size - first_node_index
            length_of_subpath = torch.randint(low=min_subpath_len, high=max_len + 1, size=[1]).item()
        ...
        index_2, _ = torch.cat((new_sulution_ascending, new_sulution_ascending), dim=1).type(torch.long).sort(dim=-1, descending=False)
        ...
        # ===== ***clamp must be before gather ***=====
        index_2 = index_2.clamp(0, problems_size - 1)
        index_3 = index_3.clamp(0, embedding_size - 1)
