...
from LEHD_TSP_p2.TSP.TSPEnv_inTSPlib import TSPEnv as Env
...
class TSPTester():
    ...
    def run(self):
        ...
        while episode < test_num_episode:
            ...
            score_teacher, score_student,problems_size,problem_name = self._test_one_batch(episode,batch_size,clock=self.time_estimator_2)
            current_gap = (score_student-score_teacher)/score_teacher

            if problems_size<200:
                problems_0_200.append(current_gap)
            ...
            elif 1000<=problems_size<5000:
                problems_1000_5000.append(current_gap)
            ...
            self.logger.info(f'problems_0_200 mean gap:, {np.mean(problems_0_200)},num problems:{len(problems_0_200)}')
            self.logger.info(f'problems_200_500 mean gap:, {np.mean(problems_200_500)},num problems:{len(problems_200_500)}')
            self.logger.info(f'problems_500_1000 mean gap:, {np.mean(problems_500_1000)},num problems:{len(problems_500_1000)}')
            self.logger.info(f'problems_1000_5000 mean gap:, {np.mean(problems_1000_5000)},num problems:{len(problems_1000_5000)}')
            ...
            if all_done:
                if not self.env_params['test_in_tsplib']:
                    ...
                    self.logger.info(" *** Test Done *** ")
                    all_result_gaps = problems_0_200 + problems_200_500 + problems_1000_5000 +problems_500_1000
                    average_gap = np.mean(all_result_gaps)
                    self.logger.info(" Average Gap: {:.4f}%".format(average_gap*100))
                    gap_ = average_gap

        return score_AM.avg, score_student_AM.avg, gap_, result_dict,[np.mean(problems_0_200),np.mean(problems_200_500),np.mean(problems_500_1000),np.mean(problems_1000_5000)]
    ...
    def _test_one_batch(self, episode, batch_size,clock=None):
        ...
        with torch.no_grad():
            ...
            for bbbb in range(budget):
                ...
                while not done:
                    if current_step == 0:
                        selected_teacher = self.env.solution[:, -1]  # The last node point is given and fixed
                        selected_student = self.env.solution[:, -1]

                    elif current_step == 1:
                        selected_teacher = self.env.solution[:, 0]  # current node this point is dynamic
