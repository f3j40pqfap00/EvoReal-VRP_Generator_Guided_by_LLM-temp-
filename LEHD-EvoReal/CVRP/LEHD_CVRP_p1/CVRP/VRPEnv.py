...
class VRPEnv:
    ...
    def load_problems(self, episode, batch_size, ):
        ...
        # shape (B,V+1)
        self.Batch_capacity_feature = self.Batch_capacity.unsqueeze(1).expand(-1,self.problems_nodes.shape[1])
        self.problems = torch.cat((self.problems_nodes,self.Batch_demand[:,:,None],
                                   self.Batch_capacity_feature[:,:,None]),dim=2)
        # shape (B,V+1,4)
        ...
        self.batch_start_capacity = self.raw_data_capacity[episode:episode+batch_size]
    ...
    def step(self, selected, selected_student,selected_flag_teacher,selected_flag_student):
        ...
        if is_depot.any():
            self.problems[is_depot, :, 3] = self.Batch_capacity[is_depot].unsqueeze(1).float()
        ...
        if smaller_.any():
            self.problems[smaller_, :, 3] = self.Batch_capacity[smaller_].unsqueeze(1).float()
