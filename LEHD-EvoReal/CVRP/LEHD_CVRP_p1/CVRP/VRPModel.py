...
class VRPModel(nn.Module):
    ...
    def forward(self, state, selected_node_list, solution, current_step,raw_data_capacity=None,):
        ...
        if self.mode == 'train':
            self.capacity = raw_data_capacity
        else:
        ...
        def probs_to_selected_nodes(probs_,split_line_,batch_size_):
            ...
            return selected_node_student_, selected_flag_student_ # The index of node starts from 1s
        ...
        if self.mode == 'test':
            ...
            is_via_depot_student = selected_node_student >= split_line  # Nodes with index greater than customer_num are through depot
...
class CVRP_Decoder(nn.Module):
    ...
    def forward(self, data,selected_node_list,capacity,remaining_capacity):
        ...
        if self.mode == 'train':
            remaining_capacity = remaining_capacity.reshape(batch_size_V,1,1)/capacity.view(-1,1,1)
        else:
