...
class VRPEnv:
    ...
    def load_problems(self, episode, max_batch_size):
        """
        Load the problem at the given episode index and expand it to batch_size copies.
        """
        self.episode = episode
        self.batch_size = max_batch_size
        

        # Only take one raw problem each time
        loc = self.raw_data_nodes[episode]         # shape (V+1, 2)
        demand = self.raw_data_demand[episode]          # shape (1, V+1)
        capacity = self.raw_data_capacity[episode]      # shape (1,)
        node_flag = self.raw_data_node_flag[episode]           # shape (1, V, 2)


        self.problems_nodes = torch.as_tensor(loc)
        self.Batch_demand = torch.as_tensor(demand)
        self.Batch_capacity = torch.as_tensor([capacity])
        self.solution = torch.as_tensor(node_flag)
        
        self.problem_size = self.problems_nodes.shape[0]-1
        
        
        # keep the batch size for all problem sizes the same
        if self.problem_size < 500:
            self.problem_batch_size = max_batch_size
        elif self.problem_size >= 500 and self.problem_size<650:
            self.problem_batch_size = int(max_batch_size/1)
        elif self.problem_size >=650 and self.problem_size<800:
            self.problem_batch_size = int(max_batch_size/1)
        else:
            self.problem_batch_size = 1   
        # Expand to batch_size dimension
        self.problems_nodes = self.problems_nodes.unsqueeze(0).expand(self.problem_batch_size, -1, -1)         # (B,V+1,2)

        self.Batch_demand = self.Batch_demand.unsqueeze(0).expand(self.problem_batch_size, -1)                 # (B,V+1)
        self.Batch_capacity = self.Batch_capacity.expand(self.problem_batch_size)                 # (B,)
        self.solution = self.solution.unsqueeze(0).expand(self.problem_batch_size, -1, -1)                     # (B,V,2)

        # Keep the capacity and shape of each sample consistent
        self.Batch_capacity = self.Batch_capacity[:, None].repeat(1, self.solution.shape[1] + 1)  # (B,V+1)
        assert self.problems_nodes.shape[1] == self.Batch_demand.shape[1] == self.Batch_capacity.shape[1], \
            f"problems_nodes: {self.problems_nodes.shape}, Batch_demand: {self.Batch_demand.shape}, Batch_capacity: {self.Batch_capacity.shape}"

        self.problems = torch.cat(
            (self.problems_nodes, self.Batch_demand[:, :, None], self.Batch_capacity[:, :, None]), dim=2
        )  # (B,V+1,4)

        if self.sub_path:
            self.problems, self.solution = self.sampling_subpaths(self.problems, self.solution)

        self.problem_size = self.problems_nodes.shape[1] # # Dynamic assignment
        self.start_capacity_value = self.Batch_capacity[0, 0].item()
        return self.problem_batch_size, self.problem_size
    ...
    def load_raw_data(self,episode=None):
        ...
        if self.env_params['mode']=='train':
            ...
            self.raw_data_nodes = self.raw_data_nodes_1 + self.raw_data_nodes_2
            self.raw_data_capacity = self.raw_data_capacity_1 + self.raw_data_capacity_2
            self.raw_data_demand = self.raw_data_demand_1 + self.raw_data_demand_2
            self.raw_data_cost = self.raw_data_cost_1 + self.raw_data_cost_2
            self.raw_data_node_flag = self.raw_data_node_flag_1 + self.raw_data_node_flag_2
    ...
    def reset(self, mode, sample_size = 1):
        ...
        self.selected_node_list = torch.zeros((self.problem_batch_size, 0), dtype=torch.long)
        self.selected_teacher_flag = torch.zeros((self.problem_batch_size, 0), dtype=torch.long)
        self.selected_student_list = torch.zeros((self.problem_batch_size, 0), dtype=torch.long)
        self.selected_student_flag= torch.zeros((self.problem_batch_size, 0), dtype=torch.long)
    ...
    def step(self, selected, selected_student,selected_flag_teacher,selected_flag_student):
        ...
        self.problems[is_depot, :, 3] =  self.start_capacity_value
        ...
        self.problems[smaller_, :, 3] =  self.start_capacity_value
